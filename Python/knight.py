...
# this function checks if the position of the knight (x,y) presents inside the board
# and if its value are equal to zero so that it is legal to put a new value into it
...
# pos_x and pos_y are all possible moves that the knight kan take
    ...
    for i in range(8):
        ...
        if inBoard(board, new_x, new_y):
                ...
                #and the counter <=65
                if solve(board,new_x,new_y,counter):
